...
evalExpr (Pow  v1) = evalExpr v1 * evalExpr v1
evalExpr (Sqrt v1) = evalExpr v1 `div` evalExpr v1
evalExpr (Val  v1)     = v1
...
plusParser ('-':rest) = Right (Sub, rest)
plusParser ('*':rest) = Right (Mul, rest)
plusParser ('/':rest) = Right (Div, rest)
